...
router.post('/addStudent', (req,res,next) => {
    ...
    users.findOne({
        ...
        if(student) {
            return res.status(400).json({
                name: 'Student Name already exists'
            });
        } else {
            const addStudent = new users({
                name: req.body.name,
                email: req.body.email,
                school_id: req.body.school_id,
                address: req.body.address,
                stu_department: req.body.stu_department,
                stu_course: req.body.stu_course,
                gender: req.body.gender,
                phoneNo: req.body.phoneNo,
                role: "Student"
            });
...
router.get('/viewStudentDetail/:id', (req,res,next) =>{
    users.findById(req.params.id, function (err, result) {
        if (err) return next(err);
        res.send(result);
    })
...
//edit Student
router.get('/editStudent/:id',(req,res,next) =>{
    users.findById(req.params.id, function (err, result) {
        if (err) return next(err);
        res.send(result);
    })
